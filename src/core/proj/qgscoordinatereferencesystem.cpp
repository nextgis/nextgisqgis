...
bool QgsCoordinateReferenceSystem::createFromString( const QString &definition )
{
  ...
  bool result = false;
  const thread_local QRegularExpression reCrsId( QStringLiteral( "^(epsg|esri|osgeo|ignf|zangi|iau2000|postgis|internal|user|mapbasic)\\:(\\w+)$" ), QRegularExpression::CaseInsensitiveOption );
  ...
  if ( match.capturedStart() == 0 )
  {
    ...
    else if ( authName == QLatin1String( "esri" ) || authName == QLatin1String( "osgeo" ) || authName == QLatin1String( "ignf" ) || authName == QLatin1String( "zangi" ) || authName == QLatin1String( "iau2000" ) || authName == QLatin1String( "mapbasic" ) )
    {
      result = createFromOgcWmsCrs( definition );
    }
    else
