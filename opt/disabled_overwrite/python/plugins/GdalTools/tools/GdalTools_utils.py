...
def setMacOSXDefaultEnvironment():
    ...
    gdal_base_path = u"%s/Library/Frameworks/gdal.framework/Versions/%s" % (qgis_app, gdal_versionsplit[0])
    ...
    if os.path.exists(qgis_standalone_gdal_path):  # qgis standalone
        ...
        if getGdalBinPath() == '':
            setGdalBinPath(qgis_bin)
        if getGdalPymodPath() == '':
            setGdalPymodPath(u"%s/Library/Python/%s.%s/site-packages" % (qgis_app, sys.version_info[0], sys.version_info[1]))
        if getHelpPath() == '':
