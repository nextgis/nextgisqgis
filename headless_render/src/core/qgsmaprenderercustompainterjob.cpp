...
{
    qDebug() << "QgsMapRendererCustomPainterJob::start()";

  if ( isActive() )
    return;

    qDebug() << "QgsMapRendererCustomPainterJob active";
...
{
    qDebug() << "QgsMapRendererCustomPainterJob::doRender()";
  ...
  qDebug() << "jobs size:" << mLayerJobs.size();
...
//      job.img = new QImage({777, 679}, QImage::Format_ARGB32_Premultiplied);
    ...
    if ( ! hasSecondPass && job.img )
    {
      ...
      job.img->save("/home/user/0.png", "PNG");
    }
